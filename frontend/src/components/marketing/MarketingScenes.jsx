 ...
 * (ivory + navy). Framed product mockups + a realistic device + a
 * concrete "automated follow-up" feed. No abstract orbs/metal.
 * ─────────────────────────────────────────────────────────────── */

/** Premium framed container for a product mockup: subtle neutral shadow + scale-in. */
...
export const AlertFeed = ({ className = "" }) => {
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <div className="pointer-events-none absolute -inset-8 rounded-[40px] blur-[80px] bg-bronze/5 opacity-70" />
