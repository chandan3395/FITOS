          ...
          <div className="space-y-2 mb-3">
            {[["Bench Press", "4 × 10", true], ["Incline Press", "3 × 12", true], ["Cable Fly", "3 × 15", false], ["Triceps", "4 × 12", false]].map(([n, s, done]) => (
              <Card key={n} className="flex items-center gap-2.5 py-2.5">
                <span className={`w-4 h-4 rounded-[5px] flex items-center justify-center shrink-0 ${done ? "bg-primary text-on-primary" : "border border-text-muted/50"}`}>{done && <CheckCircleIcon size={11} />}</span>
          ...
          <div className="space-y-2">
            {[["client", "Hit all my workouts this week 💪"], ["coach", "Amazing, Priya! Macros on point too."], ["client", "Add cardio on rest days?"], ["coach", "Let's do 2 × 20-min walks."]].map(([from, text], i) => (
              <div key={i} className={`flex ${from === "coach" ? "justify-end" : "justify-start"}`}>
                <span className={`max-w-[82%] px-3 py-2 text-[11.5px] leading-snug rounded-2xl ${from === "coach" ? "bg-primary text-on-primary rounded-br-sm" : "bg-surface-elevated border border-border text-text-primary rounded-bl-sm"}`}>{text}</span>
              </div>
            ))}
          </div>
        </>
  ...
  <div className="relative mx-auto w-[268px] sm:w-[288px]">
    ...
    <div className="relative rounded-[2.75rem] bg-surface border border-white/12 p-2.5 shadow-card-lg">
      ...
      <div className="relative rounded-[2.25rem] bg-bg overflow-hidden">
        <div className="absolute top-2.5 left-1/2 -translate-x-1/2 w-20 h-5 rounded-full bg-black z-10 border border-border" />
...
const FeatureShowcase = () => {
  ...
  useEffect(() => {
    if (!inView || hovered !== null || prefersReducedMotion()) return undefined;
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className={`text-[12px] font-mono transition-colors duration-300 ${isActive ? "text-primary" : "text-text-muted"}`}>
